...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202507261641-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  WTFPL
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2025 Jason Hempstead, Casjays Developments
# @@Created          :  Saturday, Jul 26, 2025 16:41 EDT
# @@File             :  install.sh
# @@Description      :  Install configurations for ghostty
...
__run_finalize() {
  ...
  arch="$(uname -m)"
  if [ "$arch" = "x86_64" ]; then
    url="https://github.com/wez/ghostty/releases/latest/download/ghostty-x86_64-unknown-linux-musl.tar.gz"
  elif [ "$arch" = "aarch64" ]; then
    url="https://github.com/wez/ghostty/releases/latest/download/ghostty-aarch64-unknown-linux-musl.tar.gz"
  else
  ...
  if command -v sudo >/dev/null && sudo -n true 2>/dev/null; then
    ...
    sudo install -m755 ghostty /usr/local/bin/ghostty
  else
    echo "👤 No sudo detected — installing to ~/.local/bin"
    mkdir -p "$HOME/.local/bin"
    install -m755 ghostty "$HOME/.local/bin/ghostty"
  fi
  echo "✅ Done"
  popd >/dev/null || return
  rm -rf "$tmp"
